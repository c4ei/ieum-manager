...
export async function buildVoucherPayout(privateKey,{nonce,to,value}){if(!validVoucherAddress(to))throw new Error('올바르지 않은 IEUM 수령 주소입니다.');const wallet=new Wallet(privateKey),raw=await wallet.signTransaction({type:0,chainId:21004,nonce:Number(nonce),to,value:BigInt(value),gasLimit:21_000n,gasPrice:1n});return{from:wallet.address,nonce:Number(nonce),raw,expectedHash:keccak256(raw)};}
