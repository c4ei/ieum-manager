...
export function assertSnsClaimUnique(claim,claims){
  const duplicate=claims.find(item=>item.status!=='rejected'&&(item.userId===claim.userId||item.address===claim.address||item.postUrl===claim.postUrl||`${item.platform}:${item.account}`===`${claim.platform}:${claim.account}`));
