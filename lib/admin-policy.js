...
const defaults=()=>({version:2,nodes:{},waf:{mode:'block',scoreThreshold:10,blockTtlSeconds:3600,rules:{},quarantine:{}}});
...
export async function savePolicy(policy){
  ...
  const current=await loadPolicy();const next={...current,...policy,version:2,nodes:policy.nodes||current.nodes||{},waf:{...current.waf,...(policy.waf||{})},updatedAt:new Date().toISOString()};
