...
async function adminApi(req,res,url){
  ...
  if(req.method==='POST'&&url.pathname==='/api/admin/trial-campaigns'){
    ...
    await query('INSERT INTO ieum_trial_budget_events(campaign_id,amount_wei,reason,actor) VALUES($1,$2,$3,$4)',[id,budget.toString(),'최초 캠페인 예산',actor]);await audit({action:'trial-campaign-created',ip,actor,publicId,name,budgetWei:budget.toString()});return json(res,201,{campaign:{...trialView(row),claimUrl:`${voucherPublicUrl}/trial/${publicId}`}});
  }
...
async function trialPublicApi(req,res,url){
  if(!await dbReady())return json(res,503,{error:'잠시 후 다시 시도해 주세요.'});const match=url.pathname.match(/^\/api\/trial\/([A-F0-9]{12})(?:\/claim)?$/);if(!match)return json(res,404,{error:'체험 캠페인을 찾을 수 없습니다.'});
  ...
  if(req.method==='GET')return json(res,200,{campaign:trialView(campaign),turnstileSiteKey:campaign.captcha_required?turnstileSiteKey:''});
