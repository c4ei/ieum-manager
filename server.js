...
async function adminApi(req,res,url){
  ...
  if(req.method==='GET'&&url.pathname==='/api/admin/vouchers'){
    ...
    const [items,count,summary]=await Promise.all([query(`SELECT id,public_id,amount_wei,status,expires_at,claimed_address,payout_tx_hash,payout_nonce,payout_expected_hash,claim_attempts,last_error,created_at,claimed_at,updated_at,cancelled_at,cancelled_by,cancellation_reason,print_access_count,print_accessed_at,(secret_ciphertext IS NOT NULL) AS can_reprint FROM ieum_vouchers${clause} ORDER BY created_at DESC LIMIT ${limit} OFFSET ${(page-1)*limit}`,values),query(`SELECT count(*)::int AS count FROM ieum_vouchers${clause}`,values),query("SELECT count(*)::int AS count,coalesce(sum(amount_wei),0)::text AS issued_wei,coalesce(sum(amount_wei) FILTER(WHERE status='claimed'),0)::text AS claimed_wei,coalesce(sum(amount_wei) FILTER(WHERE status IN ('issued','claiming')),0)::text AS outstanding_wei FROM ieum_vouchers")]);
    const s=summary.rows[0],total=count.rows[0].count,pages=Math.max(Math.ceil(total/limit),1);return json(res,200,{items:items.rows.map(item=>({...item,amount:formatVoucherAmount(item.amount_wei)})),summary:{count:s.count,issued:formatVoucherAmount(s.issued_wei),claimed:formatVoucherAmount(s.claimed_wei),outstanding:formatVoucherAmount(s.outstanding_wei)},pagination:{page:Math.min(page,pages),pages,total,limit},configured:Boolean(voucherPrivateKey&&voucherRpcUrl&&voucherPepper.length>=32&&voucherArchiveKey.length>=32)});
  }
  if(req.method==='POST'&&url.pathname==='/api/admin/vouchers'){
    ...
    const created=[];for(let i=0;i<quantity;i++){let inserted=false;for(let attempt=0;attempt<5&&!inserted;attempt++){const secret=createVoucherSecrets(voucherPepper),claimUrl=`${voucherPublicUrl}/voucher/${secret.publicId}?token=${encodeURIComponent(secret.token)}`,imageUrl=`/api/vouchers/${secret.publicId}/image.svg?token=${encodeURIComponent(secret.token)}&download=1`,secretCiphertext=encryptVoucherSecret({code:secret.code,token:secret.token},voucherArchiveKey);try{await query('INSERT INTO ieum_vouchers(id,public_id,code_hash,token_hash,amount_wei,expires_at,secret_ciphertext) VALUES($1,$2,$3,$4,$5,$6,$7)',[secret.id,secret.publicId,secret.codeHash,secret.tokenHash,amountWei.toString(),expiresAt?.toISOString()||null,secretCiphertext]);created.push({publicId:secret.publicId,amount:formatVoucherAmount(amountWei),code:secret.code,claimUrl,imageUrl,printUrl:`/voucher-print.html?id=${secret.publicId}&token=${encodeURIComponent(secret.token)}#code=${secret.code}`});inserted=true;}catch(error){if(error.code!=='23505')throw error;}}if(!inserted)throw new Error('중복되지 않는 상품권 번호 생성에 실패했습니다. 다시 시도하세요.');}
