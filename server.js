...
async function adminApi(req,res,url){
  ...
  if(req.method==='GET'&&url.pathname==='/api/admin/trial-campaigns'){
    ...
    return json(res,200,{campaigns:campaigns.rows.map(row=>({...trialView(row),claimCount:row.claim_count,paidCount:row.paid_count,claimUrl:`${voucherPublicUrl}/trial/${row.public_id}`,cardPngUrl:`/api/trial/${row.public_id}/image.png?download=1`})),configured:Boolean(voucherPrivateKey&&voucherRpcUrl&&trialFingerprintKey.length>=32),turnstileConfigured:Boolean(turnstileSecret&&turnstileSiteKey)});
  }
...
async function trialPublicApi(req,res,url){
  if(!await dbReady())return json(res,503,{error:'잠시 후 다시 시도해 주세요.'});const match=url.pathname.match(/^\/api\/trial\/([A-F0-9]{12})(?:\/(claim|image\.png))?$/);if(!match)return json(res,404,{error:'체험 캠페인을 찾을 수 없습니다.'});
  ...
  if(req.method==='GET'&&match[2]==='image.png'){const claimUrl=`${voucherPublicUrl}/trial/${campaign.public_id}`,qr=await QRCode.toString(claimUrl,{type:'svg',margin:3,errorCorrectionLevel:'H'}),placed=qr.trim().replace(/^<svg\s/,'<svg x="760" y="150" width="360" height="360" '),name=String(campaign.name).replace(/[<>&]/g,''),reward=formatVoucherAmount(campaign.reward_wei),end=new Date(campaign.ends_at).toLocaleDateString('ko-KR',{timeZone:'Asia/Seoul'}),svg=`<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630"><defs><linearGradient id="tg" x2="1" y2="1"><stop stop-color="#071820"/><stop offset="1" stop-color="#16483d"/></linearGradient></defs><rect width="1200" height="630" rx="42" fill="url(#tg)"/><circle cx="120" cy="90" r="180" fill="#45e5b4" opacity=".1"/><text x="70" y="85" fill="#46e6b5" font-family="sans-serif" font-size="25" font-weight="700">IEUM FIRST EXPERIENCE</text><text x="70" y="165" fill="white" font-family="sans-serif" font-size="46" font-weight="800">${name}</text><text x="70" y="265" fill="#ffcf70" font-family="sans-serif" font-size="72" font-weight="800">${reward} IEUM</text><text x="70" y="320" fill="#b6d2cb" font-family="sans-serif" font-size="24">처음 만나는 IEUM을 내 지갑으로 받아보세요.</text><text x="70" y="385" fill="white" font-family="sans-serif" font-size="27" font-weight="700">1. QR 촬영  2. 지갑 준비  3. 주소 입력</text><text x="70" y="445" fill="#9fbab4" font-family="sans-serif" font-size="20">캠페인당 한 지갑 1회 · 예산 소진 시 종료</text><text x="70" y="485" fill="#9fbab4" font-family="sans-serif" font-size="20">종료 예정 ${end}</text><rect x="745" y="135" width="390" height="390" rx="24" fill="white"/>${placed}<text x="805" y="570" fill="#b6d2cb" font-family="sans-serif" font-size="18">iem.aah.name · Mainnet 21004</text></svg>`,png=await sharp(Buffer.from(svg)).png({compressionLevel:9,palette:true}).toBuffer(),filename=`IEUM_TRIAL_${cleanDownloadPart(name)}_${campaign.public_id}.png`;res.writeHead(200,{'content-type':'image/png','content-disposition':`${url.searchParams.get('download')==='1'?'attachment':'inline'}; filename="${filename}"`,'cache-control':'public, max-age=300','x-content-type-options':'nosniff'});return res.end(png);}
  ...
  if(req.method!=='POST'||!url.pathname.endsWith('/claim'))return json(res,405,{error:'허용되지 않은 요청입니다.'});if(trialFingerprintKey.length<32)return json(res,503,{error:'체험 지급 보안키가 설정되지 않았습니다.'});
  const input=await readJson(req),address=String(input.address||'').trim(),deviceId=String(input.deviceId||'').trim().slice(0,160),ip=requestIp(req),ipHash=trialHash(ip),deviceHash=trialHash(deviceId);if(!validVoucherAddress(address))return json(res,400,{error:'올바른 IEUM 지갑 주소를 입력하세요.'});if(deviceId.length<16)return json(res,400,{error:'기기 확인 정보가 없습니다. 브라우저 저장 기능을 허용한 뒤 다시 시도하세요.'});if(campaign.captcha_required&&!await verifyTurnstile(input.captchaToken,ip))return json(res,400,{error:'사람 확인(CAPTCHA)을 완료하세요.'});
  const client=await pool.connect(),claimId=randomUUID();try{await client.query('BEGIN');const locked=(await client.query('SELECT * FROM ieum_trial_campaigns WHERE id=$1 FOR UPDATE',[campaign.id])).rows[0],now=Date.now();if(locked.status!=='active'||new Date(locked.starts_at)>now||new Date(locked.ends_at)<=now)throw new Error('현재 진행 중인 체험 캠페인이 아닙니다.');if(BigInt(locked.spent_wei)+BigInt(locked.reward_wei)>BigInt(locked.budget_wei))throw new Error('캠페인 예산이 소진되었습니다.');const checks=(await client.query("SELECT count(*) FILTER(WHERE ip_hash=$2 AND created_at>=date_trunc('day',now()))::int AS ip_day,count(*) FILTER(WHERE device_hash=$3 AND created_at>=date_trunc('day',now()))::int AS device_day,count(*) FILTER(WHERE ip_hash=$2 AND created_at>now()-interval '5 minutes')::int AS burst,max(created_at) FILTER(WHERE ip_hash=$2 OR device_hash=$3) AS last_at FROM ieum_trial_claims WHERE campaign_id=$1 AND status NOT IN ('rejected')",[locked.id,ipHash,deviceHash])).rows[0];if(checks.ip_day>=locked.ip_daily_limit)throw new Error('이 네트워크의 오늘 체험 지급 횟수를 초과했습니다.');if(checks.device_day>=locked.device_daily_limit)throw new Error('이 기기의 오늘 체험 지급 횟수를 초과했습니다.');if(checks.burst>=locked.burst_limit)throw new Error('짧은 시간에 요청이 많습니다. 잠시 후 다시 시도하세요.');if(checks.last_at&&now-new Date(checks.last_at).getTime()<locked.minimum_wait_seconds*1000)throw new Error(`${locked.minimum_wait_seconds}초 후 다시 시도하세요.`);await client.query('INSERT INTO ieum_trial_claims(id,campaign_id,address,ip_hash,device_hash,status) VALUES($1,$2,$3,$4,$5,$6)',[claimId,locked.id,address.toLowerCase(),ipHash,deviceHash,'queued']);await client.query('UPDATE ieum_trial_campaigns SET spent_wei=spent_wei+reward_wei,updated_at=now() WHERE id=$1',[locked.id]);await client.query('COMMIT');}catch(error){await client.query('ROLLBACK').catch(()=>{});if(error.code==='23505')return json(res,409,{error:'이 지갑은 이 캠페인의 체험 IEUM을 이미 신청했습니다.'});return json(res,409,{error:error.message});}finally{client.release();}
  const run=async()=>{if(!voucherPrivateKey||!voucherRpcUrl)throw new Error('체험 지급 지갑이 설정되지 않았습니다.');const provider=new JsonRpcProvider(voucherRpcUrl,21004,{staticNetwork:true}),wallet=new Wallet(voucherPrivateKey),client=await pool.connect();let signed;try{await client.query('SELECT pg_advisory_lock($1)',[210040006]);const chainNonce=await provider.getTransactionCount(wallet.address,'pending'),nonceRows=await client.query('SELECT greatest(coalesce((SELECT max(payout_nonce) FROM ieum_vouchers),-1),coalesce((SELECT max(payout_nonce) FROM ieum_trial_claims),-1))::text AS nonce'),nonce=Math.max(chainNonce,Number(nonceRows.rows[0].nonce)+1);signed=await buildVoucherPayout(voucherPrivateKey,{nonce,to:address,value:campaign.reward_wei});await client.query("UPDATE ieum_trial_claims SET status='paying',payout_nonce=$2,payout_raw_tx=$3,payout_expected_hash=$4,updated_at=now() WHERE id=$1",[claimId,nonce,signed.raw,signed.expectedHash]);const returned=(await rpc(voucherRpcUrl,'eth_sendRawTransaction',[signed.raw])).result;if(String(returned).toLowerCase()!==signed.expectedHash.toLowerCase())throw new Error('노드가 반환한 거래 해시가 예상값과 다릅니다.');await client.query("UPDATE ieum_trial_claims SET status='paid',payout_tx_hash=$2,paid_at=now(),updated_at=now() WHERE id=$1",[claimId,signed.expectedHash]);await audit({action:'trial-paid',ip,publicId:campaign.public_id,address,txHash:signed.expectedHash});return signed.expectedHash;}catch(error){await query("UPDATE ieum_trial_claims SET status='failed',last_error=$2,updated_at=now() WHERE id=$1",[claimId,String(error.message||error).slice(0,500)]).catch(()=>{});throw new Error(`지급 상태 확인이 필요합니다. 관리자에게 신청번호 ${claimId}를 알려주세요.`);}finally{await client.query('SELECT pg_advisory_unlock($1)',[210040006]).catch(()=>{});client.release();}};
  const payout=voucherPayout.then(run,run);voucherPayout=payout.catch(()=>{});const txHash=await payout;return json(res,200,{ok:true,txHash,explorerUrl:`${voucherExplorerUrl}${txHash}`});
}
