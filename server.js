...
async function adminApi(req,res,url){
  ...
  if(req.method==='GET'&&url.pathname==='/api/admin/vouchers'){
    ...
    const [items,summary]=await Promise.all([query('SELECT id,public_id,amount_wei,status,expires_at,claimed_address,payout_tx_hash,payout_nonce,payout_expected_hash,claim_attempts,last_error,created_at,claimed_at,updated_at FROM ieum_vouchers ORDER BY created_at DESC LIMIT 500'),query("SELECT count(*)::int AS count,coalesce(sum(amount_wei),0)::text AS issued_wei,coalesce(sum(amount_wei) FILTER(WHERE status='claimed'),0)::text AS claimed_wei,coalesce(sum(amount_wei) FILTER(WHERE status IN ('issued','claiming')),0)::text AS outstanding_wei FROM ieum_vouchers")]);
